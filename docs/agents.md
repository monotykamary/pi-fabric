...
Inside the guest, `agents.handoff()` resolves to `{ scheduled: true, status: "deferred", boundary: "fabric_exec_end" }`; child output cannot be consumed by later code in that same Fabric invocation. At the outer boundary, Main's tool result is replaced with the compact completion `{ handedOff, completed, status, agent, implementation, error? }`. `model` is required, the target runner is Pi, and `worktree` is intentionally unavailable because implementation must remain visible in the caller's workspace. Optional fields are `task`, `name`, `transport`, `thinking`, `tools`, `timeoutMs`, `extensions`, `recursive`, `schema`, and `compact`. The source session is not switched or historically rewritten.

**Trajectory compaction.** `compact` hands the executor a compacted transcript instead of the full raw branch: `true` applies the default summary, and `{ instructions?, preserve? }` customizes it with bounded compaction instructions (8K chars) and up to 16 explicit preserve facts (2K chars each), the same bounds as `compact.request`. The child session is written with a Fabric deterministic compaction entry appended before the boundary result, so the executor's context opens with the projected summary, followed by the kept tail from the computed cut point, then the outer `fabric_exec` result. The append-only child file retains the complete raw branch underneath the compaction marker, and the outcome (`applied`, sections, tokens, cut point — or the skip reason) is recorded on the child's `pi-fabric-handoff` custom entry under `compaction`. Omit `compact` to keep the verbatim fork.
...
5. When the continuation settles, Fabric compacts the session with the configured compaction engine unless `prewalk.compactOnReturn` is `false`, then restores the Main model captured at the boundary.

In-place mode does not spawn a child, does not fork the transcript, and does not require `agents.enabled`. When the continuation settles, Fabric compacts the session (configurable, see `prewalk.compactOnReturn`) and restores the Main model captured at the boundary; because Pi's public extension model switch also updates the default model setting, the restore repairs the default as well. Switching can fail before continuation if the model is unavailable or unauthenticated, in which case the outer result clearly reports the failure and Main remains idle.
...
Fabric passes `--tools <allowlist>` (or `--no-tools` for an empty allowlist) per run, which takes precedence over persona tool frontmatter — the built-in read-only personas ship `tools: none`, while `worker` ships `tools: all` together with `sandbox: workspace-write`. Fabric does not pass `--sandbox`, so the sandbox comes from persona frontmatter (`worker` agents can modify files). The `navigator-plan` persona additionally requires a `<program>` design block and `worker` a `<worker_report>`; failures of either protocol are surfaced as run errors, so `navigator-chat` remains the default for free-form tasks.

Each run uses an isolated `fabric-<run-id>` Veda session (`-S` + `--no-sel`), so parallel children never share selection or conversation state. Veda stores these under `.veda/sessions/` at the project root (falling back to `~/.config/veda` outside git repositories); this repository's `.gitignore` covers `.veda/`.

Veda children are **not recursively Fabric-equipped** (`recursive: true` is rejected), steering is unsupported (steer and follow-up calls throw at call time), and Veda cannot back persistent actors — it executes one headless prompt per invocation. Use `runner: "pi"` for recursive Fabric or persistent coordination.
