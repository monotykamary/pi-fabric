...
- `"project"` (default) keeps a shared actor registry at `.pi/fabric/mesh/actors/`, so actors survive `/new`. The participant directory chooses each live execution owner; other sessions keep passive views and reload on takeover. When the creating host's lease is gone and no live owner is advertised, a later trusted session whose residency claim matches the actor (Main for `session` actors, the resident host for `durable` actors) adopts it, rebinds `rootId`, and persists the registry. Registry writes are lock-serialized and merge only locally owned actor records.
