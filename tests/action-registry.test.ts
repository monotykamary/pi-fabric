...
describe("ActionRegistry", () => {
  ...
  it("keeps large write content args for previews while truncating other string args", async () => {
    const registry = new ActionRegistry();
    registry.register({
      name: "writer",
      description: "Writer",
      ...
      async invoke(_name, args) {
        return { ok: true, output: `wrote ${typeof args.path === "string" ? args.path : ""}` };
      },
    });
    ...
    await registry.invoke(
      "writer.write",
      { path: "a.md", content: "x".repeat(5000), note: "z".repeat(3000) },
      { ...context, approve: async () => {}, audits, maxResultChars: 10_000 },
    );

    expect(audits[0]?.args?.content).toBe("x".repeat(5000));
    expect((audits[0]?.args?.note as string).length).toBe(2001);
    expect(audits[0]?.args?.path).toBe("a.md");

    const audits2: FabricCallAudit[] = [];
    await registry.invoke(
      "writer.write",
      { path: "b.md", content: "y".repeat(20_000) },
      { ...context, approve: async () => {}, audits: audits2, maxResultChars: 10_000 },
    );
    const content2 = audits2[0]?.args?.content as string;
    expect(content2.length).toBe(16 * 1024 + 1);
    expect(content2.endsWith("…")).toBe(true);
  });
});
